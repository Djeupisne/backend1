...
# ══════════════════════════════════════════════════════════════════════════
# 🏦 BANQUES vs MICROFINANCE vs HORS SECTEUR
# ══════════════════════════════════════════════════════════════════════════
...
def normalize_spaces(text):
    ...
    bank_fixes = {
        ...
        r'\bo\s+r\s+a\s+b\s+a\s+n\s+k\b': 'ORABANK',
        r'\bf\s+i\s+n\s+a\s+d\s+e\s+v\b': 'FINADEV',
        r'\bg\s+l\s+s\b': 'GLS',
        r'\bw\s+o\s+r\s+l\s+d\s+v\s+i\s+s\s+i\s+o\s+n\b': 'WORLD VISION',
        r'\be\s+n\s+c\s+o\s+b\s+a\s+t\b': 'ENCOBAT',
    }
    ...
    try:
        ...
        for table in doc.tables:
            for row in table.rows:
                cells = [normalize_spaces(cell.text) for cell in row.cells if cell.text.strip()]
                if cells:
...
def detect_institution_type(text):
    ...
    if COMMERCIAL_BANK_PATTERN.search(text_lower):
        return 'commercial_bank' if not MICROFINANCE_PATTERN.search(text_lower) else 'microfinance'
    ...
    if poste in banking_posts:
        ...
        if criterion in banking_criteria:
            ...
            for match in banking_matches:
                ...
                if NON_FINANCIAL_PATTERN.search(window.lower()):
                    continue
                return True
            return False
    if poste == "Archiviste (Administration Crédit)" and criterion in ["Expérience en banque ou juridique"]:
        banking_matches = list(COMMERCIAL_BANK_PATTERN.finditer(text_lower))
        ...
        for legal in legal_terms:
            if legal in text_lower:
                ...
                if any(t in window.lower() for t in ['contrat', 'garantie', 'documentation', 'archive']):
                    return True
        return False
    if poste == "IT Réseau & Infrastructure" and criterion == "Exposition à environnement critique":
        critical = re.compile('|'.join(['banque', 'bancaire', 'bank', 'banking', 'telco', 'telecom', 'télécom', 'opérateur', 'datacenter', 'centre de données', 'data center', 'hébergement', 'hosting', 'cloud provider', 'faa', 'gouvernement', 'ministère', 'défense', 'hôpital', 'santé', 'critical infrastructure', 'ecobank', 'orabank', 'uba', 'mtn', 'airtel', 'salam', 'financial services', 'telecommunications', 'critical systems']), re.IGNORECASE)
        return bool(critical.search(text_lower))
    return True
...
def has_experience_years_strict(full_raw_text, min_years, domain_keywords=None, poste=None):
    ...
    for block in blocks:
        ...
        if poste in banking_posts:
            ...
            critical = re.compile('|'.join(['banque', 'bancaire', 'bank', 'banking', 'telco', 'telecom', 'télécom', 'opérateur', 'datacenter', 'centre de données', 'data center', 'hébergement', 'hosting', 'cloud provider', 'faa', 'gouvernement', 'ministère', 'défense', 'hôpital', 'santé', 'critical infrastructure', 'ecobank', 'orabank', 'uba', 'mtn', 'airtel', 'salam', 'financial services', 'telecommunications', 'critical systems']), re.IGNORECASE)
            if not critical.search(block.lower()):
...
def check_criterion_match_advanced(criterion, normalized_text, raw_full_text="", tokens=None, poste=None):
    ...
    if poste:
        if not check_criterion_context(criterion, raw_full_text, poste):
            print(f"    [CTX-] {criterion}: Échec contexte sectoriel")
            return False, 0.0, []
    ...
    for kw in keywords:
        ...
        if RAPIDFUZZ_AVAILABLE and len(kw_clean) >= 4:
            ...
            if ratio >= 85 and not contains_negative_context(raw_full_text, kw):
        ...
        if kw_tokens and text_tokens:
            ...
            if len(common) >= max(2, len(kw_tokens) * 0.7) and not contains_negative_context(raw_full_text, kw):
...
def analyze_cv_against_grille(cv_text, lettre_text, attestation_texts_list, poste):
    if not cv_text or len(cv_text.strip()) < 50:
        return {'score':0,'checklist':{},'flags_eliminatoires':['CV non analysable'],'signaux_detectes':[],'details':{'error':'CV vide ou non parsé'},'score_breakdown':{'bloc1_eliminatoire':True,'score_final':0,'note':'CV non analysable'}}
...
def generate_excel_report(candidats_data, poste_filter=None):
    ...
    postes_to_export = [poste_filter] if poste_filter and poste_filter in POSTES else list(dict.fromkeys(c.get('poste','') for c in candidats_data if c.get('poste') in POSTES))
    if not postes_to_export:
        ...
        for poste in postes_to_export:
            ...
            hfill, hfont = PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid"), Font(color="000000", bold=True, size=11)
            border = Border(left=Side(style='thin'), right=Side(style='thin'), top=Side(style='thin'), bottom=Side(style='thin'))
            ws.merge_cells('A1:L1'); c = ws['A1']; c.value = f"CANDIDATURES - {poste}"; c.font = Font(bold=True, size=14, color="000000"); c.alignment = Alignment(horizontal='center', vertical='center'); c.fill = hfill; ws.row_dimensions[1].height = 30
            ...
            for col, h in enumerate(headers, 1):
                cell = ws.cell(row=3, column=col, value=h); cell.font = hfont; cell.fill = hfill; cell.border = border; cell.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            for row_i, cand in enumerate(candidats_poste, 4):
                ...
                for col, val in enumerate(row_data, 1):
                    ...
                    if col == 12:
                        rec_color = get_recommandation_color(total); cell.font = Font(bold=True, color="000000")
...
def generate_csv_report(candidats_data, poste_filter=None):
    out = io.StringIO(); w = csv.writer(out, delimiter=';', quoting=csv.QUOTE_ALL, quotechar='"')
    ...
    w.writerow(headers)
    candidats_filtered = [c for c in candidats_data if c.get('poste') == poste_filter] if poste_filter and poste_filter in POSTES else candidats_data
    ...
    for idx, c in enumerate(candidats_filtered, 1):
        sb = c.get('score_breakdown_parsed', {}); score = int(c.get('score', 0)); reco = get_recommandation_from_score(score)
...
def generate_pdf_report(candidats_data, poste_filter=None):
    ...
    buf = io.BytesIO(); doc = SimpleDocTemplate(buf, pagesize=landscape(A4), rightMargin=1*cm, leftMargin=1*cm, topMargin=2*cm, bottomMargin=2*cm); els = []; sty = getSampleStyleSheet()
    ...
    for poste in postes_to_export:
        ...
        for idx, c in enumerate(candidats_poste, 1):
            score = int(c.get('score', 0)); num_dos = c.get('numero_dossier', '') or '–'; reco = get_recommandation_from_score(score)
        ...
        tbl.setStyle(TableStyle(tbl_style)); els.append(tbl); els.append(Spacer(1, 0.5*cm))
...
def login():
    ...
    email = data.get('email','').strip().lower(); pwd = hash_pwd(data.get('password',''))
...
def postuler():
    try:
        nom = (request.form.get('nom') or '').strip(); prenom = (request.form.get('prenom') or '').strip(); email = (request.form.get('email') or '').strip().lower(); telephone = (request.form.get('telephone') or '').strip(); poste = (request.form.get('poste') or '').strip(); numero_dossier = (request.form.get('numero_dossier') or '').strip()
        if not nom or not prenom or not email or poste not in POSTES: return jsonify({'error':'Champs obligatoires manquants ou poste invalide'}), 400
        for k in redis_client.keys("candidat:*"):
            existing = redis_client.hgetall(k)
            if existing.get('email') == email and existing.get('poste') == poste: return jsonify({'error':f'Vous avez déjà soumis une candidature pour le poste "{poste}".'}), 409
        def save_file(field, suffix):
            f = request.files.get(field)
            if f and f.filename and allowed_file(f.filename):
                ext = f.filename.rsplit('.',1)[-1].lower(); fn = f"{uuid.uuid4().hex}_{suffix}.{ext}"; f.save(os.path.join(UPLOAD_FOLDER, fn)); return fn
            return ''
        cv_filename = save_file('cv','cv'); lettre_filename = save_file('lettre','lettre'); att_filenames = []
        for f in request.files.getlist('attestation'):
            if f and f.filename and allowed_file(f.filename):
                ext = f.filename.rsplit('.',1)[-1].lower(); fn = f"{uuid.uuid4().hex}_attestation.{ext}"; f.save(os.path.join(UPLOAD_FOLDER, fn)); att_filenames.append(fn)
...
@jwt_required()
def get_stats():
    keys = redis_client.keys("candidat:*"); stats = {"total":len(keys),"en_attente":0,"retenu":0,"rejete":0,"entretien":0,"by_poste":[]}; counts = {}
    for k in keys:
...
@jwt_required()
def list_candidats():
    poste_filter = request.args.get('poste',''); statut_filter = request.args.get('statut',''); search = request.args.get('search','').lower(); min_score = request.args.get('min_score',type=int); result = []
    for k in redis_client.keys("candidat:*"):
...
def update_candidat(token):
    ...
    data = request.get_json(silent=True) or {}; statut = data.get('statut','en_attente'); note = data.get('note',''); score = str(min(10,max(0,int(data.get('score',0)))))
...
def trigger_analyze(token):
    ...
    if not data: return jsonify({'error':'Candidat introuvable'}), 404
    cv_fn = data.get('cv_filename'); lm_fn = data.get('lettre_filename'); att_raw = data.get('attestation_filenames','[]'); poste = data.get('poste')
...
def export_candidates(fmt):
    try:
        poste_filter = request.args.get('poste',''); statut_filter = request.args.get('statut',''); keys = redis_client.keys("candidat:*"); result = []
        for k in keys:
            c = redis_client.hgetall(k); c['id'] = k.split(':',1)[1]
            if poste_filter and c.get('poste') != poste_filter: continue
            if statut_filter and c.get('statut') != statut_filter: continue
            ...
            result.append(c)
        result.sort(key=lambda x:x.get('date_candidature',''),reverse=True); ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S"); poste_suffix = f"_{poste_filter.replace(' ','_')}" if poste_filter else "_global"; statut_suffix = f"_{statut_filter}" if statut_filter else ""; filename_base = f"rapport{poste_suffix}{statut_suffix}_{ts}"
        if fmt.lower() == 'csv':
...
if __name__ == '__main__':
    ...
    print(f"🔴 Market Risk: 3 critères éliminatoires seulement (Word exact)")
